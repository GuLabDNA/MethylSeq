...
```{r arrayData}
load(file.path(pathclass,"Data/array_titration/cancersamp_arrayEPICv2_hg38_s2.RData"))

test_betas$CG <- rownames(test_betas)
array_k562.100ng <- as.data.frame(test_betas[,c("CG","207831650102_R01C01")])
array_k562.250ng <- as.data.frame(test_betas[,c("CG","207831650102_R02C01")])


epicv2 <- as.data.frame(fread(file.path(pathref,"diffmeth", 
...
array_TF211 <- as.data.frame(test_betas[,c("CG","207831650102_R03C01")])
...
array_TF640 <- as.data.frame(betas_CGcolumn[,c("CG","207680760164_R05C01")])
...
array_TF642 <- as.data.frame(betas_CGcolumn[,c("CG","207680760164_R07C01")])
...
```{r}
pathtemp <- "D:/Jingru/deconv/Data/XR_OtherSample/OtherSample_covFiles"

f="TF211s2XR2122_comb"
TF211_1 <- fread(file.path(pathtemp,
...
f="TF640XR2122_comb"
TF640_1 <- fread(file.path(pathtemp,
                     paste0(f,"-merged.cov.SNPfiltered_clean.txt")),
                     quote="",sep="\t")
...
f="TF642XR2122_comb"
TF642_1 <- fread(file.path(pathtemp,
                     paste0(f,"-merged.cov.SNPfiltered_clean.txt")),
                     quote="",sep="\t")
...
load(file.path(pathtemp,"K56210ngXR21A_comb-hg38_PE/interFiles",
...
# Make heatmaps, array vs. XR

```{r array vs. XR}
source(file.path("D:/Jingru/deconv/Program/R_fixed","correlation_plot2_clean.R"))

# correlation between XR and microarray for the FFPE TF's, 
# Array vs. wgbs, array vs. XR
...
f="K56210ngXR21A_comb"
rep10ng_1 <- fread(file.path(pathtemp,
                     paste0(f,"-merged.cov.SNPfiltered_clean.txt")),
                     quote="",sep="\t")
...
#check 2 RRBS
rrbs_cor <- CorrelatePlot2(samp1=k562_rrbs2_chr, samp2=k562_rrbs_3,
                           assay_xr="RRBS_comb_processed", assay="RRBS_comb", covnum=15) 
rrbs_cor2 <- rrbs_cor + ggtitle("Pearson's r = 0.99")+
  theme(plot.title = element_text(hjust = 0.5))
# 0.9178573 0.9876355 


pm_wgbs.xr.othr2 <- ggarrange(wgbs_rrbsv2_2,xr_rrbsv2_2,wgbs_xrbs_2,xr_xrbs_2,
                 common.legend = TRUE, legend= "bottom",
                 nrow = 1)
pm_wgbs.xr.othr2
...
# dev.new()
# pdf(file=file.path(pathref,"Output/correlation",
#                    paste0(str_sub(Sys.Date(), start = 3), "-WGBS.XR_vsRR101nt.XRBS_15cov.pdf")),
