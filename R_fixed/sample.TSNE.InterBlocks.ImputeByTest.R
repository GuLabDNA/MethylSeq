#-----------------------------------------------------------------------------------
# script to sample.TSNE.InterBlocks.Impute based on RealsampXR_array_inter,  
# to get TSNE plots 
...
sample.TSNE.InterBlocks.ImputeByTest <- 
  function(pathtemp, TFs, ref_hg38_betas, ImpByTest_df, anno_ref, Output, filetype, samp_tp){
  ...
  for(i in 1:length(TFs)){
    ...
    test_samp_m <- merge(ImpByTest_df, samp_inter_beta, by="CG",all.x=T) 

    hg38_betas_mat <- data.matrix(test_samp_m, rownames.force = NA) #matrix is faster
    
    if(exists(".Random.seed")) rm(.Random.seed)
    #matrix with genes in the rows, samples in the columns
    betas_trans_imputed <- impute::impute.knn(hg38_betas_mat[,-1],
       k = 5, rowmax = 0.2, colmax = 0.5, maxp = 1000, rng.seed=363607815)
    #betas_trans_imputed$rng.seed # should be 362436069
    #betas_trans_imputed$rng.state # should be NULL
    betas_imp <- betas_trans_imputed$data
    a_imp <- as.data.frame(betas_imp[,dim(betas_imp)[2]])
    
    impute_cnt <- dim(betas_imp)[1]-dim(samp_inter_beta)[1]
    cg_num <- data.frame(BF_id=TFs[i], interFixedXR_CpG=dim(samp_inter_beta)[1],
                         tsne_CpG=dim(betas_imp)[1], impute_CpG=impute_cnt)
    write.csv(cg_num, file.path(pathtemp, Output, 
                                paste0(TFs[i],"_",filetype,"cpg.csv")),row.names = FALSE)
    
    a_imp$CG <- test_samp_m$CG
    colnames(a_imp)[1] <- TFs[i]

    hg38_betas_mat.imp <- merge(ref_hg38_betas,a_imp,by="CG")
    
    hg38_betas_mat <- data.matrix(hg38_betas_mat.imp, rownames.force = NA) #matrix is faster
    ...
    temp1=sapply(str_split(TFs[i], "X",  n = 2), `[`, 1)
    temp2=ifelse(str_sub(temp1,-1,-1)=="v", as.numeric(gsub("\\D", "", temp1)),
                 substring(temp1, 3))
    temp3=paste0(substr(TFs[i],1,2),temp2)
    ...
    anno3 <- data.frame(TCGA_ID = TFs[i],
                        ...
                        material = "Frozen",
                        TCGA_Project = temp3,
                        TCGA_Project_subgrp = temp3,
                        TCGA_Project_mcf = temp3,
                        shape_grp=2,
                        purity.abs=0)
    ...
    if(samp_tp=="TCGA60k"){
      ...
      meth3 <- factor(tsne_plot.df$cluster, order=T,
                      levels=c(temp3,"ACC","BLCA","BRCA_B","BRCA_L","CESC_AD",
     ...
     #"THCA_Classical","THCA_CpGislandMeth","THCA_Follicular", 
      ...
      col_temp <- setNames("#000000",temp3)

      col_code <- c(
        col_temp,"Low_fraction"="#a8a3a3",
      ...
      #"BRCA_Other"="#c299a4"
      #"THCA_Classical"="#87037a","THCA_CpGislandMeth"="#677864","THCA_Follicular"="#dbb3d7",
      ...
      meth3 <- factor(tsne_plot.df$cluster, order=T,
                      levels=c(temp3,"A IDH", "A IDH, HG", "ANA PA",  "ATRT, MYC", "ATRT, SHH",
      ...
      meth3_mcf <- factor(tsne_plot.df$cluster, order=T,
                          levels=c(temp3,"MCF IDH GLM", "MCF ENB","MCF MB G3G4", 
                                   ...
                                   "PITAD, ACTH", "PITAD, FSH LH", "PITAD, PRL",
                                   "PITAD, STH DNS A", "PITAD, STH DNS B",
                                   "PITAD, STH SPA", "PITAD, TSH", "PITUI", "PLASMA",
                                   "PTPR, A", "PTPR, B", "PXA",
                                   "RETB", "SFT HMPC","SUBEPN, PF",
                                   "SUBEPN, SPINE", "SUBEPN, ST")) #78
      ...
      col_code_mcf <- c(col_temp,
                        "MCF GBM"="#55c056",  "DMG, K27"="#a3f187", 
                        "ATRT, SHH"="#009cea", "ATRT, MYC"="#006ead", "ATRT, TYR"="#44aeff",
                        ...
                        "MCF SCHW"="#f7ca8a",
                        "CONTR, ADENOPIT"="#d3d3d3", "PITAD, TSH"="#12d7c7",
                        "LGG, DIG/DIA"="#cd4d00", "PITAD, STH SPA"="#41ccbe", "PITAD, STH DNS A"="#01c3bd",
                        "CONTR, WM"="#a8a8a8", "PLASMA"="#632f63",    "CHORDM"="#fa8cff",   
                        "RETB"="#ff9755", "CONTR, PONS"="#545454", "CONTR, HYPTHAL"="#7e7e7e"  
      )
      #"ATRT, MYC"="#006ead", "ATRT, SHH"="#009cea",
      #"SCHW"="#ffe1b2","SCHW, MEL"="#f7ca8a",
      pdf(file=file.path(pathtemp,Output, 
