...
immune_m <- read.xlsx(file.path(pathsampout,"Output_titration",
...
dat_text <- data.frame(
  label = c(paste0("RMSE (CpG)=",dmr_rmse1,"\n", "RMSE (frag.)=",uxm_rmse1),
            paste0("RMSE (CpG)=",dmr_rmse2,"\n", "RMSE (frag.)=",uxm_rmse2),
            paste0("RMSE (CpG)=",dmr_rmse3,"\n", "RMSE (frag.)=",uxm_rmse3),
            paste0("RMSE (CpG)=",dmr_rmse4,"\n", "RMSE (frag.)=",uxm_rmse4)),
  celltype  = c(1,2,3,4))

dat_text$celltype.f <- 
  factor(as.factor(dat_text$celltype), order=T,levels=c(1,2,3,4),
         labels=c("Bcell","Granulocyte","MonoMacrophage","Tcell"))
...
m1 <- read.xlsx(file.path(pathsampout,"Output_titration",
...
#prepare labels of ggplot
dat_text <- data.frame(
  label3 = c(paste0("RMSE(ref2)=",rmse_brca3),paste0("RMSE(ref2)=",rmse_coad3),
            paste0("RMSE(ref2)=",rmse_gbm3)),
  label2 = c(paste0("RMSE(ref1)=",rmse_brca2),paste0("RMSE(ref1)=",rmse_coad2),
            paste0("RMSE(ref1)=",rmse_gbm2)),
  grp  = c(1,2,3))

dat_text$grp.f <- factor(as.factor(dat_text$grp), order=T,
                                  levels=c(1,2,3),
                                  labels=c("Breast-Luminal-Ep","Colon-Ep","Brain"))
...
p.line9 <- ggplot(data=m1_target,
                  aes(x=Actual, y=Predicted, color=method.f)) + 
   geom_abline(intercept=0, slope = 1, color="gray", linetype="dashed", size=0.6) +
  geom_text(data = dat_text, mapping = aes(label=label2,
            x=0.1, y = 92,hjust=0,vjust=1.1),
            inherit.aes = F) +
  geom_text(data = dat_text, mapping = aes(label=label3,
            x=0.1, y = 80,hjust=0,vjust=1.1),
            inherit.aes = F) + 
  geom_point(size =1.35) + 
  geom_line(size = 0.9) +
...
WGBSsamples <- read.xlsx(file.path(pathdvdf, "subsample/Output_benign_titrations",
...
#output_test_uxm
pnorm <- read.csv(file=file.path(pathsampout, "Output_titration",
...
se.long_plot <- se.long %>%
  mutate(
         samp.f = factor(as.factor(Sample_id), order=T,
                ...
                labels=c("XR-methylSeq of Donor P2","CCGGflanks of WGBS2","WGBS2","WGBS1")),
